...
/**
 * @brief Экспериментальный метод, отправляющий только один какой-то предполагаемый
 * пакет данный в сериал порт
 * 
 */
void ICACHE_RAM_ATTR CRSF::sendPacket()
{
    uint8_t packetBuffer[] = {0xC8, 0x06, 0x2C, 0xEE, 0xEF, 0x0F, 0x00, 0x14};
        
    //uint8_t packetBuffer[] = {0x84, 0x6E, 0x97, 0x2E, 0xA8, 0xED, 0x44, 0xAC, 0xA6, 0x97, 0x97, 0xDD, 0xC8, 0xC1, 0x0F, 0xC4, 0x83, 0x1F, 0x3F, 0x78, 0xD0, 0x03};

    //uint8_t packetBuffer[] = {0x60, 0xA1, 0x8B, 0x2B, 0x84, 0xF7, 0xFF, 0x79, 0xFF, 0xFF, 0xED, 0x6F, 0xF8};

    //uint8_t packetBuffer[] = {0x60, 0xA1, 0x8B, 0x2B, 0x84, 0xF7, 0xFF, 0x79, 0xF5, 0xFF, 0xFB, 0xC5, 0x71, 0x00};

    //uint8_t packetBuffer[] = {0x80, 0x85, 0xDE, 0x6C, 0x61, 0xF8, 0xFF, 0xFF, 0x79, 0xF5, 0xFF, 0xFB, 0xC5, 0x71, 0x00};

    //uint8_t packetBuffer[] = {0x84, 0xBE, 0xDA, 0x08, 0xF8, 0xFF, 0x69};

    //uint8_t packetBuffer[] = {0x84, 0xBE, 0xDA, 0x08, 0xF8, 0xFD, 0xA3};

    // uint8_t packetBuffer[] = {0x00, 0x85, 0xB6, 0x6D, 0x61, 0xF8, 0xFF, 0xE1, 0xE5, 0x75, 0x23, 0x3D, 0x3B, 0x27, 0x35, 0xBF, 0x51, 0x2D, 0x73, 0x2D, 0xFF, 0xFB, 0x6F, 0x54, 0x4B, 0xAE, 0xE9, 0xDB, 0x55, 0x23, 0x23, 0x2D, 0x23, 0x31, 0xA3, 0xA3, 0xA3, 0xFF, 0x15, 0xF0};

    // uint8_t packetBuffer[] = {0xC8, 0xC1, 0x0F, 0xC4, 0x83, 0x1F, 0x3F, 0x78, 0xD0, 0x06};
    

    size_t sizePacketBuffer = sizeof(packetBuffer)/sizeof(packetBuffer[0]);
    CRSF::Port.write(packetBuffer, sizePacketBuffer);
    // #ifdef DEBUG_PRINT
    //     log_e("sizeof packetBuffer = %u ", sizePacketBuffer);
    // #endif
}
